...
export default function Timeline() {
  ...
  async function curtir(tweetId: string,index:number) {
    // const indexTweet = tweet.findIndex((t) => t.id === tweetId);
...
{tweet
  ...
  .reverse()
  .map((i, index) => {
    // Encontre os likes que correspondem ao tweet atual
    const likesForTweet = likes.filter((like) => like.tweetId === i.id);
    ...
    return (
      <div key={index}>
       ...
       <svg
        ...
        <p>{likesForTweet.length === 0 ? null : likesForTweet.length }</p>
       </div>
        <hr />
      </div>
    );
